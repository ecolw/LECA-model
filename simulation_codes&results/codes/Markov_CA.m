...
startdata_name=[tiffpath 'L2020.tif']; %%%初期数据的文件名
enddata_name=[tiffpath 'L2022.tif'];
ref_filename='L2020.tif';
...
for clcc=1:circle
    ...
    if num_growp >=0
       % for repi = 1:rep 
            ...
            while iteration < iters
                ...
                P = neighmat.*suit.*water.*markov;%.*jinzhi_E;%.*final_layer1;   %  ".*"为矩阵元素对应相乘，"*"为矩阵相乘
    ...
    num_growl=endda-start;     iter_grow=round(num_growl/iters);  %(末期城市元胞数-基期)/迭代次数=每次迭代增长的元胞数，round为四舍五入到最近的整数。 
    if num_growl>=0
        nsize=3;
            ...
            while iteration < iters
               ...
               % P = neighmat.*pos2.*suit.*water2.*markov;
                P = neighmat.*suit.*water.*markov;%.*jinzhi.*yunxu;%.*final_layer4;   %  ".*"为矩阵元素对应相乘，"*"为矩阵相乘
                simudata=SHOLD(P,iter_grow,simudata,2);
                if iteration == iters        %最后一次迭代
                    neighmat = ORNE1(simudata,nsize,2);
                    suit = simudata;suit(suit~=2) = 1;suit(suit==2) = 0;
                   % P = neighmat.*pos2.*suit.*water2.*markov;
                    P = neighmat.*suit.*water.*markov;%.*jinzhi.*yunxu;%.*final_layer4;
    ...
    if num_growe >=0
            ...
            while iteration < iters
                ...
                if iteration == iters        %最后一次迭代
                    ...
                    suit = simudata;suit(suit~=3) = 1;suit(suit==3) = 0;
                    %P = neighmat.*pos3.*suit.*water2.*markov;
                    P = neighmat.*suit.*water.*markov;%.*jinzhi_E;%.*final_layer4;
...
output_filename = [resultdata_name '2022testtestmarkovsimudata_iter' num2str(clcc) '.tif'];  % 保存文件名
